...
const getFiles = async () => {
  ...
  mediaSettingsStore.restaurantId = Number(localStorage.getItem('RestaurantId'))
  mediaSettingsStore.slideTransitionInterval = Number(localStorage.getItem('SlideTransitionInterval'))
  const aspectRatioId = calculateAspectRatio(mediaSettingsStore.screenSize)
  await mediaSettingsStore.getCustomerDisplayFile({restaurant_id: mediaSettingsStore.restaurantId, aspect_ratio_id: aspectRatioId, group_ids: String(mediaSettingsStore.group)})
}

onBeforeMount(async () => {
  const localSocketServerIp = localStorage.getItem('localSocketServerIp')
  const localSocketServerPort = Number(localStorage.getItem('localSocketServerPort'))
